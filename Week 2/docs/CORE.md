...
- `pauseTask/resumeTask` — `ACTIVE↔PAUSED` (не для DONE) (`core/src/ChatService.kt:586`); `nextStep/prevStep/completeCurrentStep` — навигация/пометка шагов (`core/src/ChatService.kt:600`); `setTaskSteps(taskId, titles)` — замена шагов (≤10, ≤80 симв) (`core/src/ChatService.kt:636`); `updateNextAction(taskId, action)` (≤200 симв) (`core/src/ChatService.kt:643`); `setTaskStage(taskId, stage)` — прямой прыжок только по валидному переходу (`core/src/ChatService.kt:565`)
